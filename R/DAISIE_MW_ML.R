    ...
    if (min(pars1) < 0) {
      ...
      if (max(pars1new[c(1, 2, 4, 5)]) > 1E+10) {
        ...
        if (parallel != "no") {
          ...
          X <- NULL; rm(X)
          loglik <- foreach::foreach(X = datalist, .combine = sum, .export = c("pars2"), .packages = c("DAISIE", "foreach", "deSolve", "doParallel"))  %dopar%  DAISIE_loglik_CS(X[[1]]$pars1new, pars2, X)
        } else {
          loglik <- 0
          if (pars2[4] == 0.5) pb <- utils::txtProgressBar(min = 0, max = length(datalist), style = 3)
          for (i in 1:length(datalist)) {
            loglik <- loglik + DAISIE_loglik_CS(pars1 = datalist[[i]][[1]]$pars1new, pars2 = pars2, datalist = datalist[[i]], methode = methode, CS_version, abstolint = abstolint, reltolint = reltolint)
